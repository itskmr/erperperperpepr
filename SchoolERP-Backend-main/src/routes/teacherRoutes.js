...
import { protect, authorize, enforceSchoolIsolation, validateSchoolOwnership, addSchoolFilter } from '../middlewares/authMiddleware.js';
...
// GET all teachers for a school - PROTECTED WITH SCHOOL ISOLATION
router.get('/school/:schoolId', 
  ...
  async (req, res) => {
    try {
      ...
      const whereClause = addSchoolFilter(req, { schoolId: parseInt(schoolId) });
...
// GET search and filter teachers - PROTECTED WITH SCHOOL ISOLATION
router.get('/school/:schoolId/search', 
  protect, 
  authorize('admin', 'school', 'teacher'), 
  enforceSchoolIsolation,
  async (req, res) => {
    try {
      ...
      let baseWhere = { schoolId: parseInt(schoolId) };
...
// GET a single teacher by ID - PROTECTED WITH SCHOOL ISOLATION
router.get('/:id', 
  protect, 
  authorize('admin', 'school', 'teacher'), 
  validateSchoolOwnership('teacher', 'id'),
  async (req, res) => {
    try {
      const { id } = req.params;
...
// CREATE a new teacher - PROTECTED WITH SCHOOL ISOLATION
router.post('/', 
  protect, 
...
// UPDATE a teacher - PROTECTED WITH SCHOOL ISOLATION
router.put('/:id', 
  protect, 
  authorize('admin', 'school'), 
  validateSchoolOwnership('teacher', 'id'),
  async (req, res) => {
    try {
      ...
      const updateData = {
        ...
        ...(experience !== undefined && { experience: experience || '0' }),
        ...(joining_year !== undefined && { joining_year: joining_year ? new Date(joining_year) : new Date() })
      };
...
// DELETE a teacher - PROTECTED WITH SCHOOL ISOLATION
router.delete('/:id', 
  protect, 
  authorize('admin', 'school'), 
  validateSchoolOwnership('teacher', 'id'),
